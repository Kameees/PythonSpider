...
class TWT(object):
    ...
    def reqs(self, url_items, sum_num, num = 0):
        ...
        if res.status_code == 200:
            ...
            try:
                gs_con = con_et.xpath('.//div[@class="right2"]/ul/li')
    ...
    def gevent_down(self, url, item):
        ...
        try:
            ...
            nr_s = []
            A = 0
            p_s = dom_tree.xpath('.//div[@class="countiner"]/p')
            if len(p_s) == 0:
                p_s = dom_tree.xpath('.//div[@class="bor"]/div/p')
                if len(p_s) == 0:
                    p_s = dom_tree.xpath('.//div[@class="neirong"]/p')
            for x in p_s:
                try:
    ...
    def find_one(self):
        ...
        #获取每一页的详情链接 存入mongo
        for sum_num, i in enumerate([['http://io.ruc.edu.cn/more.php?cid=70', 1306]]):
             self.reqs(i, sum_num)
