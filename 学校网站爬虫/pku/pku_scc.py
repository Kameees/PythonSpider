...
class TWT(object):
    ...
    def reqs(self, url_items, sum_num, num = 0):
        ...
        if res.status_code == 200:
            ...
            try:
                gs_con = con_et.xpath('.//tbody[@id="articleList-body"]/tr')[7:]
    ...
    def gevent_down(self, url, item):
        ...
        try:
            ...
            nr_s = []
            A = 0
            p_s = dom_tree.xpath('.//div[@class="col-xs-12 content-div ke-content-wrap"]//p')
            if len(p_s) == 0:
                p_s = dom_tree.xpath('.//div[@class="col-xs-12 content-div ke-content-wrap"]//div')
            for x in p_s:
                try:
                ...
                nr_s.append(cont)
            file_list = dom_tree.xpath('.//div[@class="col-xs-12"]/a')
            for x in file_list:
                try:
                    ...
                    continue
            last_con = dom_tree.xpath('.//div[@style="padding-top: 20px;"]//text()')
            cont = "".join(last_con).strip()
            nr_s.append(cont)
            while '' in nr_s:
                nr_s.remove('')
    ...
    def find_one(self):
        ...
        #获取每一页的详情链接 存入mongo
        for sum_num, i in enumerate([['https://scc.pku.edu.cn/home!recruitList.action?category=1', 1266]]):
             self.reqs(i, sum_num)
