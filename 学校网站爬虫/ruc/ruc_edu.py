...
class TWT(object):
    ...
    def reqs(self, url_items, sum_num, num = 0):
        ...
        if res.status_code == 200:
            ...
            try:
                gs_con = con_et.xpath('.//ul[@class="noticelist"]/li')
    ...
    def gevent_down(self, url, item):
        ...
        try:
            ...
            nr_s = []
            A = 0
            p_s1 = dom_tree.xpath('.//div[@class="nc_body"]//div')
            p_s = p_s1
            if len(p_s) == 0:
                p_s = dom_tree.xpath('.//div[@class="nc_body"]//p')
                if len(p_s) == 0:
                    p_s = dom_tree.xpath('.//div[@class="content"]//p')
            for x in p_s:
                try:
    ...
    def find_one(self):
        ...
        #获取每一页的详情链接 存入mongo
        for sum_num, i in enumerate([['https://www.ruc.edu.cn/loadnoticelist?tid=3&lid=1', 1263], ['https://www.ruc.edu.cn/loadnoticelist?tid=1&lid=1', 1305]]):
             self.reqs(i, sum_num)
