...
class TWT(object):
    ...
    def reqs(self, url_items, sum_num, num = 0):
        ...
        if res.status_code == 200:
            ...
            try:
                gs_con = con_et.xpath('//div[@class="content-m"]//dt/a')
            except:
                num += 1
                self.reqs(url, num)
    ...
    def gevent_down(self, url, item):
        ...
        try:
            ...
            title = item['title']
            try:
                source = str(dom_tree.xpath('//table[@class="content-m-m"]//td[@align="center"]/text()')[1]).split('文章来源：')[1].split("阅读次数：")[0].strip()
            except:
                source = '杭州师范大学钱江学院'
            author = ''
            ...
            for x in p_s:
                try:
                    ...
                    if str(img_list_one_).find("default/images/icon")>-1:
                        #是链接不是图片
                        pdf_url_ = x.xpath('.//a/@href')[0]
                        pdf_url = urljoin(url, pdf_url_)
                        pdf_titles = x.xpath('.//a//text()')[0]
    ...
    def find_one(self):
        ...
        #获取每一页的详情链接 存入mongo
        for sum_num, i in enumerate([['http://jxb.hznu.edu.cn/572/list.htm', 1305]]):
             self.reqs(i, sum_num)
