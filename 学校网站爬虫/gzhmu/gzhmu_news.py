...
class TWT(object):
    ...
    def reqs(self, url_items, sum_num, num = 0):
        ...
        if res.status_code == 200:
            ...
            try:
                gs_con = con_et.xpath('.//*[@id="newlist"]/div')[1:21]
    ...
    def gevent_down(self, url, item):
        ...
        try:
            ...
            title = dom_tree.xpath('.//*[@id="newmainleft1"]/div[2]/div[1]/text()')[0].strip()
            bt2 = str(dom_tree.xpath('.//*[@id="newmainleft1"]/div[2]/div[2]/text()')[0]).split('\n\t\t')
            try:
                author = str(bt2[2]).split("作者:")[1].strip()
                if len(author) > 20:
                    author = author[:17] + '...'
            except:
                author = ""
            try:
                source = str(bt2[1]).split("来源:")[1].strip()
            except:
                source = "广州医科大学"
            p_s = dom_tree.xpath('.//*[@id="newmainleft1"]/div[2]/div[3]/div')
            if len(p_s) == 0:
                p_s = dom_tree.xpath('.//*[@id="newmainleft1"]/div[2]/div[3]//p')
            bendi_img_address = []
            nr_s = []
            A = 0
            for x in p_s:
    ...
    def find_one(self):
        # 访问第一页
        # 获取每一页的详情链接 存入mongo
        for sum_num, i in enumerate([['https://online.gzhmu.edu.cn/news/newslist--111.html', 1306], ['https://online.gzhmu.edu.cn/news/newslist--113.html', 1306], ['https://online.gzhmu.edu.cn/news/newslist--112.html', 1306], ['https://online.gzhmu.edu.cn/news/newslist--114.html', 1306], ['https://online.gzhmu.edu.cn/news/newslist--115.html', 1306]]):
             self.reqs(i, sum_num)
