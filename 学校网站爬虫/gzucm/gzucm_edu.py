...
class TWT(object):
    ...
    def reqs(self, url_items, sum_num, num = 0):
        ...
        if res.status_code == 200:
            ...
            try:
                gs_con = con_et.xpath('.//*[@class="content_list"]/ul/li')
                for con_li in gs_con:
                    url_one = con_li.xpath('./a/@href')[0].strip()
    ...
    def gevent_down(self, url, item):
        ...
        try:
            ...
            nr_s = []
            A = 0
            p_s = dom_tree.xpath('.//div[@id="vsb_content_1001"]//p')
            if len(p_s) == 0:
                return
            for x in p_s:
                try:
    ...
    def find_one(self):
        #获取每一页的详情链接 存入mongo
        for sum_num, i in enumerate([['http://www.gzucm.edu.cn/index/xydt.htm', 1306], ['http://www.gzucm.edu.cn/index/mtbd.htm', 1306]]):
             self.reqs(i, sum_num)
