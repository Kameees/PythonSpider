...
class TWT(object):
    ...
    def gevent_down(self, url, item):
        ...
        try:
            ...
            A = 0
            #   图片
            img_l = dom_tree.xpath('.//span[@id="pagepicimgsu9"]/span')
            for x in img_l:
                try:
                    img_list_one_ = x.xpath('./text()')[0].strip()
            ...
            #   文字
            try:
                c_s = dom_tree.xpath('.//span[@id="pagepiccontentsu9"]/textarea/text()')[0].split('>')
                conts = []
                for c in c_s:
                    if '</span' in c:
                        con = c.split('</span')[0].strip()
                        conts.append(con)
            except:
                conts = []
    ...
    def find_one(self):
        ...
        #获取每一页的详情链接 存入mongo
        for sum_num, i in enumerate([['http://gdutnews.gdut.edu.cn/liebiaotupian.jsp?urltype=tree.TreeTempUrl&wbtreeid=1113', 1306]]):
             self.reqs(i, sum_num)
