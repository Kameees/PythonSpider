...
class TWT(object):
    ...
    def reqs(self, url_items, sum_num, num = 0):
        ...
        if len(root) != 0:
            try:
                for item in root.iterfind('Notice'):
                    data_id = item[0].text
                    title = item[2].text
                    save = OrderedDict()
                    url_one = 'https://www.pku.edu.cn/dat/%s.html' % (data_id)
            ...
            num += 1
            self.reqs(url, sum_num, num)
        print("第 %s 页， 插入 %s 条， 重复 %s 条， 共 %s 条"%(url, self.A, self.B, len(root)))
    ...
    def gevent_down(self, url, item):
        ...
        try:
            ...
            author = ''
            nr_s = []
            p_s = dom_tree.xpath('.//section[@id="notice_content"]//p')
            for x in p_s:
                try:
    ...
    def find_one(self):
        ...
        #获取每一页的详情链接 存入mongo
        for sum_num, i in enumerate([['https://www.pku.edu.cn/dat/recentList.xml', 1305]]):
             self.reqs(i, sum_num)
