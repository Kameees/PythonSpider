...
#   暨南要闻 + 部门快讯 + 院系速递 + 最新媒体暨大 + 学者视角 + 学苑 + 学子
...
class TWT(object):
    ...
    def reqs(self, url_items, sum_num, num = 0):
        ...
        if num > 4:
            return None
        res = requests.get(url, headers=headers, verify=True, allow_redirects=False, timeout=10)
        gs_con = []
        if res.status_code == 200:
            con_et = etree.HTML(res.content.decode('utf-8', 'ignore'))
            try:
                gs_con = con_et.xpath('.//div[@class="mainContent"]/ul/li')
                for con_li in gs_con:
                    try:
                        try:
                            data_id = con_li.xpath('./a/@href')[0].strip()
                        except:
                            data_id = con_li.xpath('.//div[@class="title"]/a/@href')[0].strip()
                        try:
                            title = con_li.xpath('./a/text()')[0].strip()
                        except:
                            title = con_li.xpath('.//div[@class="title"]/a/text()')[0].strip()
                    except:
                        continue
                    save = OrderedDict()
    ...
    def gevent_down(self, url, item):
        ...
        try:
            ...
            title = item['title']
            try:
                source = dom_tree.xpath('.//*[@id="content"]/div[1]/div[1]/div[1]/span[4]/text()')[0].split('来源: ')[1].strip()
            except:
                source = '暨南大学'
            try:
                author = dom_tree.xpath('.//*[@id="content"]/div[1]/div[1]/div[1]/span[5]/text()')[0].split('作者: ')[1].strip()
                if len(author) > 20:
                    author = author[:17] + '...'
            except:
                author = ''
            ...
            A = 0
            #   图片 + 文字
            p_s = dom_tree.xpath('.//*[@id="fontzoom"]/p')
            for x in p_s:
                try:
                    try:
                        img_list_one_ = x.xpath('./img/@src')[0].strip()
                    except:
                        img_list_one_ = x.xpath('./input/@src')[0].strip()
                    if 'file:///C' in img_list_one_:
                        continue
    ...
    def find_one(self):
        ...
        #获取每一页的详情链接 存入mongo
        for sum_num, i in enumerate([['https://news.jnu.edu.cn/jnyw/List/List_149.html', 1306], ['https://news.jnu.edu.cn/xzsj/', 1306], ['https://news.jnu.edu.cn/xysx/bmkx/', 1306], ['https://news.jnu.edu.cn/xysx/yxsd/', 1306], ['https://news.jnu.edu.cn/mtjd/dt/', 1306], ['https://news.jnu.edu.cn/xshc/xt/', 1306], ['https://news.jnu.edu.cn/xshc/ll/', 1306]]):
             self.reqs(i, sum_num)
