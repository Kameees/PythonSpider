...
class TWT(object):
    ...
    def reqs(self, url_items, sum_num, num = 0):
        ...
        if res.status_code == 200:
            ...
            try:
                gs_con = con_et.xpath('.//div[@class="articleList01"]/ul/li')
                for con_li in gs_con:
                    data_id = con_li.xpath('.//h3/a/@href')[0].strip()
                    title = con_li.xpath('.//h3/a/text()')[0].strip()
                    save = OrderedDict()
    ...
    def gevent_down(self, url, item):
        ...
        try:
            ...
            title = item['title']
            try:
                source = dom_tree.xpath('.//div[@class="articleAuthor"]/p/span[2]/text()')[0].split('信息来源：')[1].strip()
            except:
                source = '北京大学'
            author = ''
            bendi_img_address = []
            nr_s = []
            A = 0
            p_s = dom_tree.xpath('.//div[@class="article"]//p')
            for x in p_s:
                try:
    ...
    def find_one(self):
        ...
        #获取每一页的详情链接 存入mongo
        for sum_num, i in enumerate([['http://pkunews.pku.edu.cn/xwzh/index.htm', 1306], ['http://pkunews.pku.edu.cn/ldhd/index.htm', 1306], ['http://pkunews.pku.edu.cn/mtbdnew/index.htm', 1306], ['http://pkunews.pku.edu.cn/jxky/index.htm', 1306], ['http://pkunews.pku.edu.cn/bdrw/index.htm', 1306], ['http://pkunews.pku.edu.cn/wyyd/xwdt/index.htm', 1306], ['http://pkunews.pku.edu.cn/wyyd/xxyg/index.htm', 1306]]):
             self.reqs(i, sum_num)
