...
class TWT(object):
    ...
    def reqs(self, url_items, sum_num, num = 0):
        ...
        if res.status_code == 200:
            ...
            try:
                gs_con = con_et.xpath('//ul[@class="newListContent"]/li')
            except:
                num += 1
                self.reqs(url, num)
            for con_li in gs_con:
                data_id = con_li.xpath('./@onclick')[0].strip()
                title = con_li.xpath('./a//text()')[0].strip()
                save = OrderedDict()
                url_one_ = str(data_id).split("location.href='")[1].replace("'",'')
                ...
                try:
                    ...
                    if sum_num == 0:
                        ...
                        self.gevent_down_two(url_one, save)
                    self.A += 1
    ...
    def gevent_down(self, url, item):
        ...
        try:
            ...
            author = ''
            source = '浙江工商大学'
            p_s = dom_tree.xpath('//div[@class="detailsContent"]/p')
            bendi_img_address = []
            nr_s = []
            A = 0
            for x in p_s:
    ...
    def gevent_down_two(self, url, item):
        a = 0
        while True:
            a += 1
            if a > 5:
        ...
        try:
            ...
            author = ''
            source = ''
            p_s = dom_tree.xpath('//div[@class="xjhMessConList"]')[0].xpath('./p')
            bendi_img_address = []
            nr_s = []
            A = 0
            for x in p_s:
    ...
    def find_one(self):
        ...
        #获取每一页的详情链接 存入mongo
        for sum_num, i in enumerate([['http://jyw.zjgsu.edu.cn/eweb/index.so?type=newsSearch&nrdm=zpgg', 1266], ['http://jyw.zjgsu.edu.cn/eweb/index.so?type=zphxxSearch&zphtype=all&nrdm=zphxx', 1266]]):
             self.reqs(i, sum_num)
