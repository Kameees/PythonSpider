...
class TWT(object):
    ...
    def reqs(self, url_items, sum_num, num = 0):
        ...
        if res.status_code == 200:
            ...
            try:
                gs_con = con_et.xpath('.//div[@class="content"]//ul/li')[:14]
    ...
    def gevent_down(self, url, item):
        ...
        try:
            ...
            nr_s = []
            A = 0
            p_s = dom_tree.xpath('.//div[@class="wp_articlecontent"]/p')
            if len(p_s) == 0:
                p_s = dom_tree.xpath('.//div[@class="details-content"]//p')
                if len(p_s) == 0:
                    p_s = dom_tree.xpath('.//div[@class="informationContent informationContent-2621"]/p')
                    if len(p_s) == 0:
                        p_s = dom_tree.xpath('.//div[@class="informationContent informationContent-2623"]/p')
                        if len(p_s) == 0:
                            p_s = dom_tree.xpath('.//div[@class="informationContent informationContent-2624"]/p')
                            if len(p_s) == 0:
                                p_s = dom_tree.xpath('.//div[@class="informationContent informationContent-2625"]/p')
            for x in p_s:
                try:
                    href_one = x.xpath('.//a/@href')[0].strip()
                    href_one = urljoin(url, href_one)
                    href_text = x.xpath('.//a/text()')[0].strip()
    ...
    def find_one(self):
        ...
        #获取每一页的详情链接 存入mongo
        for sum_num, i in enumerate([['https://www.jnu.edu.cn/xxgg/list.htm', 1305], ['https://www.jnu.edu.cn/2624/list.htm', 1306], ['https://www.jnu.edu.cn/2625/list.htm', 1306], ['https://www.jnu.edu.cn/2623/list.htm', 1306]]):
             self.reqs(i, sum_num)
