...
class TWT(object):
    ...
    def reqs(self, url_items, sum_num, num = 0):
        ...
        if res.status_code == 200:
            ...
            try:
                gs_con = con_et.xpath('.//div[@class="content_col_2_list"]/ul/li')
    ...
    def gevent_down(self, url, item):
        ...
        try:
            ...
            title = item['title']
            try:
                source = dom_tree.xpath('.//div[@class="nc_author"]/text()')[0].split('来源：')[1].strip()
            except:
                source = '中国人民大学'
            try:
                author = dom_tree.xpath('.//div[@class="nc_author"]/text()')[1].split('编辑：')[1].strip()
            except:
                author = ''
            bendi_img_address = []
            nr_s = []
            A = 0
            p_s = dom_tree.xpath('.//div[@class="nc_body"]//p')
            for x in p_s:
                try:
                    img_list_one = x.xpath('./img/@src')[0].strip()
                    try:
                        A += 1
                        if A == 1:
    ...
    def find_one(self):
        ...
        #获取每一页的详情链接 存入mongo
        for sum_num, i in enumerate([['http://news.ruc.edu.cn/archives/category/important_news', 1306], ['http://news.ruc.edu.cn/archives/category/camp_news', 1306], ['http://news.ruc.edu.cn/archives/category/media_news', 1306]]):
             self.reqs(i, sum_num)
