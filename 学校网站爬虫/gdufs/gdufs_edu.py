...
class TWT(object):
    ...
    def reqs(self, url_items, sum_num, num = 0):
        ...
        if res.status_code == 200:
            ...
            try:
                gs_con = con_et.xpath('.//div[@class="m_content"]/ul/div/li/span')
    ...
    def gevent_down(self, url, item):
        ...
        try:
            ...
            nr_s = []
            A = 0
            p_s = dom_tree.xpath('.//div[@class="v_news_content"]//p')
            for x in p_s:
                try:
                ...
                nr_s.append(cont)
            #   附件
            file_list = dom_tree.xpath('.//*[@id="wrap"]/div[3]/div[3]/form/table/tbody/tr[7]/td/span/span')
            for f in file_list:
                try:
                    href_one = f.xpath('.//a/@href')[0].strip()
                    href_one = urljoin(url, href_one)
                    href_text = f.xpath('.//a//text()')[0].strip()
    ...
    def find_one(self):
        ...
        #获取每一页的详情链接 存入mongo
        for sum_num, i in enumerate([['https://www.gdufs.edu.cn/xsjz.htm', 1263], ['https://www.gdufs.edu.cn/tzgg.htm', 1305]]):
             self.reqs(i, sum_num)
