...
class TWT(object):
    ...
    def reqs(self, url_items, sum_num, num = 0):
        ...
        if res.status_code == 200:
            ...
            try:
                gs_con = con_et.xpath('//ul[@class="list-1 pt10"]/li/span/a')
            except:
                num += 1
                self.reqs(url, num)
    ...
    def gevent_down(self, url, item):
        ...
        try:
            ...
            author = ''
            source = '浙江工商大学'
            p_s = dom_tree.xpath('//dd[@id="the_content"]/p')
            bendi_img_address = []
            nr_s = []
            A = 0
            for x in p_s:
    ...
    def find_one(self):
        ...
        #获取每一页的详情链接 存入mongo
        for sum_num, i in enumerate([['http://news.zjgsu.edu.cn/17/', 1256], ['http://news.zjgsu.edu.cn/18/', 1256]]):
             self.reqs(i, sum_num)
