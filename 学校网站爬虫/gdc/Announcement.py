...
class TWT(object):
    ...
    def reqs(self, url_items, sum_num, num = 0):
        ...
        if res.status_code == 200:
            ...
            try:
                gs_con = con_et.xpath('.//ul[@class="List"]/li')
    ...
    def gevent_down(self, url, item):
        ...
        try:
            ...
            if len(p_s) == 1:
                p_s = dom_tree.xpath('.//div[@class="Content"]/table/tbody/tr/td/p')
            if len(p_s) == 0:
                p_s = dom_tree.xpath('.//div[@class="Content"]//section//p')
            #   图片+文字
            try:
                for x in p_s:
                    ...
                    try:
                        for img_list_one in img_list:
                            ...
                            cont = '<p><img src="%s"/></p>' % (img_list_one)
                            nr_s.append(cont)
                    except :
                        continue
                    conts = x.xpath('.//text()')
                ...
                return
            #   判断是否存在附件，存入nr_s列表中
            file_s = dom_tree.xpath('.//div[@class="file_list"]/a')
            if len(file_s) != 0:
                for l in file_s:
                    try:
    ...
    def find_one(self):
        ...
        #获取每一页的详情链接 存入mongo
        for sum_num, i in enumerate([['http://gdc.stu.edu.cn/Announcement/Indexs', 1305]]):
             self.reqs(i, sum_num)
