...
class TWT(object):
    ...
    def reqs(self, url_items, sum_num, num = 0):
        ...
        if res.status_code == 200:
            ...
            try:
                gs_con = con_et.xpath('//table[@class="winstyle197125"]//tr/td/a')
            except:
                num += 1
                self.reqs(url, num)
    ...
    def gevent_down(self, url, item):
        ...
        try:
            ...
            author = ""
            try:
                source = str(dom_tree.xpath('//div[@align="center"]/following-sibling::div[1]/text()')[0]).split("资讯来源：")[1].split("\xa0")[0].strip()
            except:
                source = "浙江中医药大学"
    ...
    def find_one(self):
        #获取每一页的详情链接 存入mongo
        for sum_num, i in enumerate([['https://news.zcmu.edu.cn/xwtt.htm', 1306]]):
             self.reqs(i, sum_num)
