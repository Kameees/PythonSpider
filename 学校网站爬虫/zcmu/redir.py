...
class TWT(object):
    ...
    def reqs(self, url_items, sum_num, num = 0):
        ...
        if res.status_code == 200:
            con_et = etree.HTML(res.content.decode('gb18030'))
            try:
                gs_con = con_et.xpath('//div[@class="list clearfix"]//li/a')
            except:
                num += 1
                self.reqs(url, num)
    ...
    def gevent_down(self, url, item):
        ...
        try:
            ...
            title = item['title']
            try:
                author = str(dom_tree.xpath('//p[@class="art-summary"]/text()')[0]).split("作者：")[1].split("\xa0")[0].strip()
            except:
                author = ""
            try:
                source = str(dom_tree.xpath('//p[@class="art-summary"]/text()')[0]).split("来源：")[1].split("\xa0")[0].strip()
            except:
                source = "浙江中医药大学"
            p_s = dom_tree.xpath('//div[@class="art-content article-content"]/p')
            if len(p_s) == 0:
                return
            bendi_img_address = []
            nr_s = []
    ...
    def find_one(self):
        #获取每一页的详情链接 存入mongo
        for sum_num, i in enumerate([['http://www.zisu.edu.cn/redir.php?catalog_id=81', 1306], ['http://www.zisu.edu.cn/redir.php?catalog_id=80', 1306], ['http://www.zisu.edu.cn/redir.php?catalog_id=234', 1263], ['http://www.zisu.edu.cn/redir.php?catalog_id=79', 1305]]):
             self.reqs(i, sum_num)
