...
class TWT(object):
    ...
    def reqs(self, url_items, sum_num, num = 0):
        ...
        if res.status_code == 200:
            ...
            try:
                gs_con = con_et.xpath('.//div[@class="cwrapper"]/ul/li')
                for con_li in gs_con:
                    data_id = con_li.xpath('.//a/@href')[0].strip()
                    title = con_li.xpath('.//a/h3/text()')[0].strip()
                    save = OrderedDict()
    ...
    def gevent_down(self, url, item):
        ...
        try:
            ...
            A = 0
            try:
                img_list_one_ = dom_tree.xpath('.//div[@class="titleimg"]/img/@src')[0].strip()
    ...
    def find_one(self):
        ...
        #获取每一页的详情链接 存入mongo
        for sum_num, i in enumerate([['https://www.tsinghua.edu.cn/publish/thu2018/11484/index.html', 1306]]):
             self.reqs(i, sum_num)
