...
class TWT(object):
    ...
    def reqs(self, url_items, sum_num, num = 0):
        ...
        if res.status_code == 200:
            ...
            try:
                gs_con = con_et.xpath('//div[@class="text-list"]/ul//li')
    ...
    def gevent_down(self, url, item):
        ...
        try:
            ...
            title = item['title']
            try:
                author = str(dom_tree.xpath('.//div[@class="detail-infor"]/span/text()')[1]).split("作者：")[1].strip()
            except:
                author = ""
            try:
                source = str(dom_tree.xpath('.//div[@class="detail-infor"]/span/text()')[0]).split("来源：")[1].strip()
            except:
                source = "广州大学"
    ...
    def find_one(self):
        # 访问第一页
        #获取每一页的详情链接 存入mongo
        for sum_num, i in enumerate([['http://news.gzhu.edu.cn/gdyw.htm', 1306], ['http://news.gzhu.edu.cn/gspdxjs.htm', 1306],['http://news.gzhu.edu.cn/xydt.htm', 1306], ['http://news.gzhu.edu.cn/msfc.htm', 1306]]):
             self.reqs(i, sum_num)
