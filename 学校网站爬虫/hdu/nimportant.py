...
class TWT(object):
    ...
    def reqs(self, url_items, num = 0):
        ...
        if res.status_code == 200:
            ...
            try:
                gs_con = con_et.xpath('//div[@class="list-box"]//div[@class="con"]//li')
            except:
                num += 1
                self.reqs(url, num)
            for con_li in gs_con:
                data_id = con_li.xpath('./@data-id')[0].strip()
                title = con_li.xpath('./div[@class="title"]/text()')[0].strip()
                save = OrderedDict()
                # 职位
                # 可能有分页问题
                try:
                    houzhui = list(filter(lambda x: str(url).find(x) > -1, ['important', 'notice', 'media']))[0]
                except:
                    return
                url_one = "http://www.hdu.edu.cn/news/%s_%s" % (houzhui, data_id)
    ...
    def gevent_down(self, url, item):
        ...
        try:
            ...
            main_c = dom_tree.xpath('//div[@class="article"]/div[@class="instr"]/text()')[0]
            if not main_c:
                return
            title = dom_tree.xpath('//div[@class="article"]/div[@class="title"]/text()')[0].strip()
            author = main_c.split('作者：')[1].split('来源')[0].strip()
            source = main_c.split('来源：')[1].split('时间')[0].strip()
            p_s = dom_tree.xpath('//div[@class="article"]/div[@class="con"]/p')
            bendi_img_address = []
            nr_s = []
            A = 0
            for x in p_s:
    ...
    def find_one(self):
        ...
        #获取每一页的详情链接 存入mongo
        for i in [['http://www.hdu.edu.cn/news/important', 1036], ['http://www.hdu.edu.cn/news/notice', 1035], ['http://www.hdu.edu.cn/news/media', 1036]]:
             self.reqs(i)
        #开始另一个链接
        url = ["http://foreign.hdu.edu.cn/list.php?cid=1", 1036]
        self.reqs_two(url)
