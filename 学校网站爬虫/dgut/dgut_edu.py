...
class TWT(object):
    ...
    def reqs(self, url_items, sum_num, num = 0):
        ...
        if res.status_code == 200:
            ...
            try:
                gs_con = con_et.xpath('.//div[@class="listList"]/ul/li')
                for con_li in gs_con:
                    data_id = con_li.xpath('.//a/@href')[0].strip()
                    title = con_li.xpath('.//a/@title')[0].strip()
                    save = OrderedDict()
    ...
    def gevent_down(self, url, item):
        ...
        try:
            ...
            source = '东莞理工学院'
            try:
                author = dom_tree.xpath('.//*[@id="zoomtime"]/ul/li[1]/text()')[0].split('发布单位：')[1].strip()
            except:
                author = ''
            bendi_img_address = []
            nr_s = []
            A = 0
            p_s = dom_tree.xpath('.//*[@id="zoomcon"]/ucapcontent/p')
            if len(p_s) == 0:
                p_s = dom_tree.xpath('.//*[@id="zoomcon"]/ucapcontent//p')
            for x in p_s:
                try:
    ...
    def find_one(self):
        ...
        #获取每一页的详情链接 存入mongo
        for sum_num, i in enumerate([['http://www.dgut.edu.cn/dgut/ggyw/news_list.shtml', 1306], ['http://www.dgut.edu.cn/dgut/xydt/news_list.shtml', 1306], ['http://www.dgut.edu.cn/dgut/gjdt/index_list.shtml', 1306], ['http://www.dgut.edu.cn/dgut/xsgg/index_list.shtml', 1263], ['http://www.dgut.edu.cn/dgut/mtgg/news_list.shtml', 1306]]):
             self.reqs(i, sum_num)
