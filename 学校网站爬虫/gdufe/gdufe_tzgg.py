...
class TWT(object):
    ...
    def reqs(self, url_items, sum_num, num = 0):
        ...
        if res.status_code == 200:
            ...
            try:
                gs_con = con_et.xpath('.//*[@id="wp_news_w6"]/ul/li')
    ...
    def gevent_down(self, url, item):
        ...
        try:
            ...
            source = '广东财经大学'
            try:
                author = dom_tree.xpath('.//*[@id="d-container"]/div/div/div/p/span[1]/text()')[0].split('发布者：')[1].strip()
            except:
                author = ''
            bendi_img_address = []
            nr_s = []
            ...
            for x in p_s:
                try:
                    href_one = x.xpath('.//a/@href')[0]
                    if 'http' in href_one:
                        href_one = x.xpath('.//a/@href')[1]
                    href_one = href_one.strip()
                    href_one = urljoin(url, href_one)
                    href_text = x.xpath('.//a/text()')[0].strip()
                    cont = '<p><a href="%s">%s</a></p>' % (href_one, href_text)
                except:
                    try:
                        img_list_one_ = x.xpath('.//img/@src')[0]
                        if '.gif' in img_list_one_:
                            img_list_one_ = []
                        img_list_one_.strip()
    ...
    def find_one(self):
        ...
        #获取每一页的详情链接 存入mongo
        for sum_num, i in enumerate([['http://www.gdufe.edu.cn/3178/list.htm', 1305]]):
             self.reqs(i, sum_num)
