...
class TWT(object):
    ...
    def reqs(self, url_items, sum_num, num = 0):
        ...
        if res.status_code == 200:
            ...
            try:
                gs_con = con_et.xpath('.//div[@class="list_column"]/ul/div/ul/li')
    ...
    def gevent_down(self, url, item):
        ...
        try:
            ...
            if len(li_s) != 0:
                 for l in li_s:
                    try:
                        style = dom_tree.xpath('.//form[@name="_newscontent_fromname"]/div/ul/@style')[0]
                    except:
                        continue
    ...
    def find_one(self):
        ...
        #获取每一页的详情链接 存入mongo
        for sum_num, i in enumerate([['http://jwc.gdut.edu.cn/ggtz.htm', 1305]]):
             self.reqs(i, sum_num)
