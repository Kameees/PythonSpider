...
headers = {
    ...
    'Cookie': 'JSESSIONID=6C86B4B5199925523F8D83F38F462F47.tomcat101; UM_distinctid=16c98587a32746-05dce3958bf065-7373e61-130980-16c98587a336ab; CNZZDATA1256277512=912460585-1565923294-%7C1565923294',
}
...
class TWT(object):
    ...
    def reqs(self, url_items, sum_num, num = 0):
        ...
        if res.status_code == 200:
            ...
            try:
                gs_con = con_et.xpath('.//div[@class="d_lb"]/div')
                for con_li in gs_con:
                    data_id = con_li.xpath('.//a/@onclick')[0].split("'")[1].strip()
                    title = con_li.xpath('.//a/text()')[0].strip()
                    save = OrderedDict()
                    url_one = 'https://career.gdufs.edu.cn/eweb/jygl/zpfw.so?modcode=jygl_zphxxck&subsyscode=zpfw&type=viewZphxx&id=%s' % (data_id)
    ...
    def gevent_down(self, url, item):
        ...
        try:
            ...
            nr_s = []
            A = 0
            p_s = dom_tree.xpath('.//td[@class="td_border_four"]//p')
            for x in p_s:
                conts = x.xpath('.//text()')
                cont = "".join(conts).strip()
                nr_s.append(cont)
            file_list = dom_tree.xpath('.//*[@id="tab"]/tbody//a')
            for x in file_list:
                try:
                    href_one = x.xpath('./@href')[0].strip()
    ...
    def find_one(self):
        ...
        #获取每一页的详情链接 存入mongo
        for sum_num, i in enumerate([['https://career.gdufs.edu.cn/eweb/jygl/zpfw.so?type=zph', 1305]]):
             self.reqs(i, sum_num)
